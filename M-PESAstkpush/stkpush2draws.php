...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['phone_number'])) {
    ...
    if (!$isValid) {
        ...
        $accountReference = '2DRAWS-' . date('Ymd-His');
        $description = 'Payment for 2 Draws Tips - Ksh ' . $AMOUNT;
...
<head>
    ...
    <title>MPESA Payment - 2 Draws Tips</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .alert-success {
            background: linear-gradient(135deg, var(--success-bg) 0%, #bae6fd 100%);
            border-color: #0ea5e9;
            color: var(--success);
        }
...
<body>
    <div class="container">
        ...
        <h1 class="title">2 Draws Tips</h1>
        <p class="subtitle">Unlock <strong>15+ odds safe draws</strong> with detailed analysis.</p>
        ...
        <div class="paybill-box">
            <span class="paybill-label">Till Number</span>
            <span class="paybill-number" id="tillNumber" title="Click to copy" onclick="copyTill()"><?= $BUSINESS_SHORT_CODE ?></span>
        </div>
    ...
    <script>
        ...
        function copyTill() {
            const tillText = document.getElementById('tillNumber').textContent;
            if (navigator.clipboard && navigator.clipboard.writeText) {
                navigator.clipboard.writeText(tillText).then(() => {
                    showToast();
                }).catch(() => {
                    fallbackCopy(tillText);
                });
            } else {
                fallbackCopy(tillText);
            }
